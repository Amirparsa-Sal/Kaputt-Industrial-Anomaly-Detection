"""
Experiment orchestration for VLM anomaly classification.

Mirrors the structure of experiment.py but adapted for the VLM pipeline:
no bounding-box outputs, binary + optional multi-class confusion matrices,
and VLM-specific config serialisation.
"""
...
# ---------------------------------------------------------------------------
# Logging helpers (own logger name, independent of SAM3)
# ---------------------------------------------------------------------------
...
def _save_pair_scores(
    ...
    """Persist raw per-(prompt, image) scores for post-hoc threshold tuning."""
    ...
    with open(out, "w") as f:
        json.dump(payload, f)
    logger.info(f"  Raw pair scores saved to {out}")
...
def _save_experiment(
    ...
    """Serialize full VLM experiment results to JSON."""
    if exp_dir is None:
    ...
    experiment = {
        ...
        "config": {
            ...
            "pad_short_references": cfg.pad_short_references,
            "temperature": cfg.temperature,
            "max_new_tokens": cfg.max_new_tokens,
            "load_in_4bit": cfg.load_in_4bit,
            "enable_thinking": cfg.enable_thinking,
            "system_prompt_text_zero_shot": cfg.system_prompt_text_zero_shot,
            "system_prompt_text_few_shot": cfg.system_prompt_text_few_shot,
            "system_prompt_logits_zero_shot": cfg.system_prompt_logits_zero_shot,
            "system_prompt_logits_few_shot": cfg.system_prompt_logits_few_shot,
            "user_prompt_text_zero_shot": cfg.user_prompt_text_zero_shot,
            "user_prompt_text_few_shot": cfg.user_prompt_text_few_shot,
            "user_prompt_logits_zero_shot": cfg.user_prompt_logits_zero_shot,
            "user_prompt_logits_few_shot": cfg.user_prompt_logits_few_shot,
        },
        "auroc": auroc_ser,
...
def run_vlm_experiment(
    ...
    """
    Run a single VLM classification experiment:
      filter data → infer → compute AUROC / AP → confusion matrices
    ...
    mode_str = f"{cfg.shot_mode} / {cfg.scoring_mode}"
    prompts_summary = f"user+system prompts ({mode_str})"
    ...
    logger.info(f"  scoring_mode:     {cfg.scoring_mode}")
    logger.info(f"  shot_mode:        {cfg.shot_mode}")
    if cfg.shot_mode == "few_shot":
        logger.info(
            f"  pad_short_references: {cfg.pad_short_references} "
            f"(cycle 1–2 refs to fill 3 panels if true)",
        )
    ...
    logger.info(f"  num_data:         "
                f"{cfg.num_data if cfg.num_data > 0 else '(all)'}")
    ...
    # ---- VLM inference ----
    max_scores, pair_scores, text_outputs = run_vlm_inference(
    ...
    logger.info(f"  Detected (score>0.5): {num_detected}")
    ...
    logger.info("=" * 60)
    logger.info(
        f"  {'Thresh':>7s}  {'TP':>5s}  {'FP':>5s}  {'FN':>5s}  {'TN':>5s}"
    ...
    # Always: binary confusion matrix
    binary_true = [
    ...
    if pair_scores:
        ...
        logger.info("=" * 60)
        logger.info(
            f"  {'Prompt':<35s} {'AP':>7s} {'AUROC':>7s} {'BestT':>7s}"
            f" {'#Pos':>6s} {'#Neg':>6s}"
        )
        logger.info("-" * 75)
        for row in per_prompt:
            ap_s = (
                f"{row['ap']:.4f}" if not np.isnan(row["ap"]) else "   N/A"
            )
            auroc_s = (
                f"{row['auroc']:.4f}"
            ...
            logger.info(
                f"  {row['prompt']:<35s} {ap_s:>7s} {auroc_s:>7s} "
                f"{bt_s:>7s} {row['num_positive']:>6d} "
                f"{row['num_negative']:>6d}"
            )
        logger.info("=" * 60)
        ...
        logger.info("=" * 60)
        logger.info(
            f"  {'Defect Type':<20s} {'Prompt':<35s} "
            f"{'Mean':>7s} {'#Samples':>8s}"
        )
        logger.info("-" * 75)
        for row in per_pair:
            logger.info(
                f"  {row['defect_type']:<20s} {row['prompt']:<35s} "
